// A complete working C program to demonstrate doubly linked list 
#include <stdio.h> 
#include <stdlib.h> 
...
{ 
	/* 1. if list in NULL or invalid position is given */
        ...
        /* 2. traverse up to the node at position 'index' from the beginning */
        for (i= 0; tmp != NULL && i< index; i++)
           tmp = tmp->next;

        /* 3. if 'index' is greater than the number of nodes in the doubly linked list */
        if (tmp == NULL)
           return;
